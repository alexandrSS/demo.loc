...
class NestedSetBehavior extends Behavior
{
    ...
    /** @var $_cached \yii\db\ActiveRecord[]|NestedSetBehavior[] */
     ...
     * @return ActiveQuery ActiveQuery of owner.
     */
    ...
    {
        $owner = $this->owner;
        return $query = $owner->find()->andWhere("[[$this->leftAttribute]] = 1");
    }
    ...
    public function delete()
    {
        ...
        if ($owner->getIsNewRecord()) {
            throw new Exception(yii::t('app', 'The node cannot be deleted because it is new.'));
        }

        if ($this->getIsDeletedRecord()) {
            throw new Exception(yii::t('app', 'The node cannot be deleted because it is already deleted.'));
        }
    ...
    public function moveAsRoot()
    {
        ...
        if (!$this->hasManyRoots) {
            throw new Exception(yii::t('app', 'Many roots mode is off.'));
        }
        ...
        if ($owner->isRoot()) {
            throw new Exception(yii::t('app', 'The node already is root node.'));
        }
     ...
     * @param ActiveRecord|NestedSetBehavior $subj the subject node.
    ...
    public function beforeSave($event)
    {
        if ($this->_ignoreEvent) {
            ...
            throw new Exception(yii::t('app', 'You should not use CActiveRecord::save() method when BbNestedSetBehavior attached.'));
        }
    }
    ...
    public function beforeDelete($event)
    {
        if ($this->_ignoreEvent) {
            ...
            throw new Exception(yii::t('app', 'You should not use CActiveRecord::delete() method when BbNestedSetBehavior attached.'));
        }
    }
    ...
    private function addNode($target, $key, $levelUp, $runValidation, $attributes)
    {
        ...
        if (!$owner->getIsNewRecord()) {
            throw new Exception(yii::t('app', 'The node cannot be inserted because it is not new.'));
        }

        if ($this->getIsDeletedRecord()) {
            throw new Exception(yii::t('app', 'The node cannot be inserted because it is deleted.'));
        }

        if ($target->getIsDeletedRecord()) {
            throw new Exception(yii::t('app', 'The node cannot be inserted because target node is deleted.'));
        }
    ...
    private function makeRoot($attributes)
    {
        ...
        if ($this->hasManyRoots) {
            ...
            if ($owner->roots()->exists()) {
                throw new Exception(yii::t('app', 'Cannot create more than one root in single root mode.'));
            }
    ...
    /**
     * Correct cache for {@link BbNestedSetBehavior::delete()} and {@link BbNestedSetBehavior::deleteNode()}.
     */
    ...
    /**
     * Correct cache for {@link BbNestedSetBehavior::addNode()}.
     * @param int $key .
     */
    private function correctCachedOnAddNode($key)
    {
        ...
        $this->owner->trigger(self::EVENT_AFTER_CHANGE_NODE, new ModelEvent());
    }

    /**
     * Correct cache for {@link BbNestedSetBehavior::moveNode()}.
     * @param int $key .
