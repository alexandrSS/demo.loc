...
use backend\modules\system\models\query\SystemEventQuery;
...
class SystemEvent extends \yii\db\ActiveRecord
{
    ...
    public function getName(){
        $names = [
            'users'=>[
                User::EVENT_AFTER_SIGNUP => Yii::t('system', 'New user')
            ]
        ];
    ...
    public function getMessage(){
        $messages = [
            'users'=>[
                User::EVENT_AFTER_SIGNUP => Yii::t('system', 'New user {username} ({email}) was registered at {created_at, date} {created_at, time}', $this->data)
            ]
        ];
